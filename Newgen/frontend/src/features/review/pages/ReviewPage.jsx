...
export default function ReviewPage() {
  ...
  const { viewMode, setViewMode, zoom, setZoom, isAnalyzing, analysisComplete, runAnalysis } = useReview()

  // redirect if no images uploaded
  useEffect(() => {
    if (!figmaImage) navigate('/')
  }, [figmaImage])
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between px-5 py-3 border-b border-[#1e1e1e] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-700">›</span>
            <span className="text-[11px] bg-[#252525] text-gray-400 border border-[#333] px-2 py-0.5 rounded font-mono">V1.2</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-0.5 gap-0.5">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-all ${
                  viewMode === 'side-by-side' ? 'bg-[#2a2a2a] text-white' : 'text-gray-500 hover:text-gray-300'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-all ${
                  viewMode === 'slider' ? 'bg-[#2a2a2a] text-white' : 'text-gray-500 hover:text-gray-300'
                }`}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <circle cx="12" cy="12" r="1"/><line x1="12" y1="2" x2="12" y2="22"/>
                </svg>
                Slider Overlay
              </button>
            </div>
            ...
            <div className="flex items-center gap-1.5 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-2 py-1.5">
              <button onClick={() => setZoom(z => Math.max(50, z - 10))} className="text-gray-500 hover:text-white text-xs w-4 text-center">−</button>
              <span className="text-xs font-mono text-gray-400 w-10 text-center">{zoom}%</span>
              <button onClick={() => setZoom(z => Math.min(200, z + 10))} className="text-gray-500 hover:text-white text-xs w-4 text-center">+</button>
            </div>
            ...
            >
              ...
              {isAnalyzing ? 'Analyzing...' : analysisComplete ? 'Re-scan with AI' : 'Run AI Scan'}
            </button>
